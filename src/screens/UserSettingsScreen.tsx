...
const UserSettingsScreen: React.FC = () => {
    ...
    */
    ...
    const navigateToManageSubscription = () => {
        if (__DEV__) console.log('[DEBUG] Attempting to navigate to UserManageSubscriptionScreen');
        try {
            navigation.navigate('UserManageSubscriptionScreen' as never);
            if (__DEV__) console.log('[DEBUG] Navigation to UserManageSubscriptionScreen successful');
        } catch (error) {
            if (__DEV__) console.error('[DEBUG] Navigation error:', error);
        }
    };
